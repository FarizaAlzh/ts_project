...
def build_report(document_text: str, previous_analysis_text: Optional[str], api_key: str) -> str:
    ...
    task_business = Task(
        ...
        description=(
            "Проведи глубокий анализ предоставленного ТЗ. Сосредоточься на следующих аспектах:\n"
            "- Критические бизнес-риски для компании\n"
            "- Целевая аудитория и уникальная ценность продукта\n"
            "- Состав MVP и приоритетность функций\n"
            "- Соответствие законодательству РК (включая закон о персональных данных)\n"
            "- Информационная безопасность\n"
            "- Список из 3-5 уточняющих вопросов к заказчику"
            "Текст ТЗ: {document_text}\n"
            "Предварительный анализ: {previous_analysis_text}"
        ),
        expected_output=(
            "Полный бизнес-анализ в формате Markdown. "
            "Используй четкую структуру: заголовки H2 для разделов, "
            "Оценка бизнес-рисков Риск, Вероятность, Влияние "
            "и маркированные списки. Весь текст должен быть на профессиональном русском языке"
        ),
    ...
    task_technical = Task(
        ...
        expected_output=("Технический отчет в формате Markdown, дополняющий бизнес-анализ. Обязательно: 1. Архитектурная схема словами. 2. Обоснование стека (ClickHouse, асинхронность). 3. Тех. риски. 4 Используй заголовки H2/H3 и профессиональный IT-сленг."),
        agent=tech_lead,
